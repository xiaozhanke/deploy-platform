...
/**
 * 验证 HostRecord 在内存 / 序列化路径里携带的敏感字段不会被 toString 或 Jackson 无意泄露。
 *
 * <p>覆盖的两层：
 * <ul>
 *   <li>{@link HostRecordDto}：服务层内部传递凭据用，{@code @ToString.Exclude} +
 *       {@code @JsonIgnore} 防止日志和 JSON 误带</li>
 *   <li>{@link HostRecord}：JPA 实体，进入懒加载日志或调试输出时也不能暴露</li>
 * </ul>
 *
 * <p>{@code HostRecordVo} 已彻底移除 password / privateKeyPassword 字段，从类型系统层面
 * 杜绝出站泄露，无需也无法再用运行时断言覆盖。
 *
 * @author xiaozhanke
 */
class HostRecordSecretMaskingTest {
    ...
    void dtoToStringMustNotLeakSecrets() {
        HostRecordDto dto = new HostRecordDto();
        dto.setName("test");
    ...
    void entityToStringMustNotLeakSecrets() {
        HostRecord entity = new HostRecord();
