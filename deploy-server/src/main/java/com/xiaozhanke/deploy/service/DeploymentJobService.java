...
public class DeploymentJobService {
    ...
    private DeploymentJobVo doCreateJob(DeploymentRecord record, CreateJobRequest request) {
        ...
        deploymentMQProducer.sendDeploymentJob(pending);

        // listener 已经把 pending 写入 DB 并 commit 半消息,此处直接转 VO 返回
        // 若 listener ROLLBACK(唯一索引冲突),再查一次拿已存在的兜底返回
        DeploymentJob persisted = deploymentJobRepository.findByDeploymentRecordIdAndJobTypeAndClientRequestId(
    ...
    public PageResult<DeploymentJobVo> queryJobs(String deploymentRecordId, JobStatusEnum status,
                                                 Pageable pageable) {
        Specification<DeploymentJob> spec = (root, query, cb) -> {
            List<Predicate> predicates = new ArrayList<>();
            predicates.add(cb.equal(root.get("deploymentRecord").get("id"), deploymentRecordId));
            if (status != null) {
                predicates.add(cb.equal(root.get("status"), status));
            }
            return cb.and(predicates.toArray(new Predicate[0]));
        };
        Page<DeploymentJob> page = deploymentJobRepository.findAll(spec, pageable);
